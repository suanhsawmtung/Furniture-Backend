...
export class AdminOrderService implements IAdminOrderService {
  async listOrders(
    ...
    return {
      success: true,
      data: {
        items: (await enrichOrders(items)) as ListOrderT[],
        currentPage,
        totalPages,
        pageSize,
      },
  ...
  async getOrderDetail(code: string): Promise<ServiceResponseT<ListOrderT>> {
  ...
  async updateOrder(
    ...
    const isStatusChanged =
      newStatus !== undefined && newStatus !== existingOrder.status;

    if (isStatusChanged) {
      const allowedTransitions =
        ORDER_STATUS_FLOW[existingOrder.status as OrderStatus];
      if (
        !allowedTransitions ||
        !allowedTransitions.includes(newStatus as OrderStatus)
      ) {
        throw createError({
      ...
      if (
        existingOrder.source === OrderSource.CUSTOMER &&
    ...
    const isNameChanged =
      ...
      customerName.trim() !== (existingOrder.customerName || "");
    const isPhoneChanged =
      customerPhone !== undefined &&
      customerPhone.trim() !== (existingOrder.customerPhone || "");
    const isAddressChanged =
      customerAddress !== undefined &&
      customerAddress.trim() !== (existingOrder.customerAddress || "");
    const isNotesChanged =
      customerNotes !== undefined &&
      (customerNotes?.trim() || "") !== (existingOrder.customerNotes || "");
    ...
    if (
      existingOrder.source === OrderSource.ADMIN &&
      isLockedStatus &&
      hasActualFulfillmentChanges
    ) {
      throw createError({
    ...
    if (
      ...
      throw createError({
        message:
          "Admin cannot modify fulfillment details for customer-originated orders.",
        status: 400,
        code: errorCode.invalid,
      });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (isItemsChanged) {
        ...
        for (const item of items) {
          ...
          const quantityNum = Number(item.quantity);
          calculatedTotalPrice += Number(item.price) * quantityNum;
      ...
      if (isStatusChanged) {
        if (newStatus === OrderStatus.DONE) {
          ...
          for (const reservation of activeReservations) {
            ...
            const avgCost =
              Number(variant.totalCost) / parseInt(String(variant.stock), 10);
